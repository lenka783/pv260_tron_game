...
public class Player extends Item {
    ...
    public void update(long timePassed,  Room room) {
        ...
        checkFood((ArenaRoom)room);
        List<CollisionMask> collisionMasks = path.stream()
                .map((point) -> new SquareCollisionMask(
                        speed,
                        new Point(point.x - getCenter().x, point.y - getCenter().y)
                ))
                .collect(Collectors.toList());
        this.setCollisionMask(new ComposedCollisionMask(collisionMasks));
        path.add(new Point(getCenter()));
    }

    private void checkFood(ArenaRoom room) {
        if (room.getFood() != null && isOverFood(room)) {
            room.setFood(null);
        } else {
            path.remove(0);
        }
    ...
    private boolean isOverFood(ArenaRoom room) {
        Point offset = new Point(room.getFood().x - getCenter().x, room.getFood().y - getCenter().y);
        CollisionMask foodCollisionMask = new SquareCollisionMask(10, offset);
        return (new SquareCollisionMask(10)).isInCollision(foodCollisionMask, offset);
    }
